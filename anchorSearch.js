...
{if (tableempty)
    ...
    link.href=pageFilename+"#"+anchor[0];
    link.style.color="rgb(87, 158, 182)";
    link.addEventListener("click",function(event){event.preventDefault();openLink(pageFilename+"#"+anchor[0]);});
    link.textContent=pageTitle+'  ';
    tdn.appendChild(link);
    tds =document.createElement("TD");
    if (anchor[1].substr(anchor[0].length-4) == "_top")
    {
        tds.textContent = "<Top of page>";
    }
    else
    {
        var sectionText = anchor[1].substr(0,1).toUpperCase() + anchor[1].substr(1) + ' ';
        sectionText = sectionText.replace("Table ", "Table: ");
        tds.textContent = sectionText;
    }
...
function insertHeader(tb)
{
    ...
    var topNotice=document.createElement("B");
    topNotice.textContent='Top results may not be the most relevant';
    thn.appendChild(topNotice);
    tr=document.createElement("TR");
    tr.appendChild(thn);
    tb.appendChild(tr);

    thn =document.createElement("TD");
    var pageBold=document.createElement("B");
    pageBold.textContent='Page  ';
    thn.appendChild(pageBold);
    ths =document.createElement("TD");
    var sectionBold=document.createElement("B");
    sectionBold.textContent='Section  ';
    ths.appendChild(sectionBold);
    tr=document.createElement("TR");
    tr.appendChild(thn);
    tr.appendChild(ths);
    tb.appendChild(tr);
...
{var safeOpener=null;
    ...
    {
        var w=window.open(lnk, "anchor_detail", "scrollbars=yes,resizable=yes, toolbar=no, status=no, location=no, directories=no, screenX=10, screenY=20, top=20, left=10,noopener,noreferrer");
        if (w)
        {
            w.focus();
        }
